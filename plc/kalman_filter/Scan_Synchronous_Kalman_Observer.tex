...
\begin{tikzpicture}[
  ...
  arr/.style={-{Stealth[length=2.0mm]}, line width=0.5pt}
]

\node[box] (in) {Inputs\\$x_k,\ \Delta t_k$};
\node[box, below=of in] (pred) {Predict\\$\hat{\mathbf{x}}^- ,\ P^-$};
\node[box, below=of pred] (upd) {Update\\$y,S,\ K$\\$\hat{\mathbf{x}},\ P$};
\node[box, below=of upd] (out) {Outputs\\$y=\hat{x}$\\$y_{\dot{}}=\hat{v}$};
...
% Feedback arrow
\draw[arr] (upd.east) -- ++(10mm,0) |- (pred.east);

\end{tikzpicture}
\caption{Predict--update cycle per PLC scan with persistent state carried between scans.}
\label{fig:compact_flow}
\end{figure}
...
{\setlength{\parindent}{2em}\indent
A deterministic ramp–hold–ramp–hold waveform was injected into the PLC at $\Delta t = 50$ ms. Measurement noise was added as bounded uniform noise:}
